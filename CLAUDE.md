...
what the `run` closure guarantees. The artwork is generated by
`crates/gitr/assets/make_icon.py`; regenerate rather than editing the PNG. Two traps that
cost a render each: an SVG gradient in `objectBoundingBox` units does not paint on a
perfectly vertical or horizontal stroke, since that bounding box has zero width or height —
use `gradientUnits="userSpaceOnUse"`; and the 1024px render embeds at 831 KB because the
grain filter defeats PNG compression, so 512 is what ships.
